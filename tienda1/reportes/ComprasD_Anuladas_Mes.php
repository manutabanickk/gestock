...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1)
        {
            ...
            $this->Cell(98);
            // Title
            $this->Cell(105, 10, 'COMPRAS (DETALLADAS) ANULADAS DEL MES ' . strtoupper($meses[(int)substr($mes, 0, 2) - 1] . ' del ' . $ano), 0, 0, 'C');
...
try {
    ...
    if (is_array($listado) || is_object($listado)) {
        foreach ($listado as $row => $column) {
            ...
            $pdf->Cell(22, 5, number_format($column["precio_unitario"], 2, '.', ','), 0, 0, 'C', 1);
            $pdf->Cell(22, 5, number_format($column["exento"], 2, '.', ','), 0, 0, 'C', 1);
            $pdf->Cell(22, 5, number_format($column["importe"], 2, '.', ','), 0, 0, 'C', 1);
            $pdf->Ln(6);
            $get_Y = $pdf->GetY();
