<?php
require __DIR__ . '/../vendor/autoload.php'; // Asegúrate de que Composer esté configurado correctamente
require_once '../model/Venta_model.php'; // Incluye el archivo que contiene la clase Venta
require_once '../controller/Venta_controller.php'; // Incluye el archivo que contiene la clase Venta (si es necesario)
...
function generarQRCode($data) {
    $tempDir = sys_get_temp_dir(); // Directorio temporal del sistema
...
try {
    ...
    $filePath = '../../qrcodesave/qr_code.png'; // Cambia esto a la ruta donde deseas guardar el código QR
    $filePath = generarQRCode($numero_venta);
    ...
    if ($tipo_comprobante == '1') {
        ...
        if ($tipo_pago == 'EFECTIVO') {
            ...
            $pdf->Image($filePath, 22, $get_Y + 54, 30, 30); // Usar la imagen QR generada
            $pdf->SetFont('Arial', 'B', 8.5);
    ...
    $pdf->Output('I', 'Ticket_' . $numero_comprobante . '.pdf');
} catch (Exception $e) {
    $pdf->Text(22.8, 5, 'ERROR AL IMPRIMIR TICKET');
    $pdf->Output('I', 'Ticket_ERROR.pdf');
}
?>
